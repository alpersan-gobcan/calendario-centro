...
export async function GET(request: Request) {
  try {
    ...
    // 2. Calculate next week's Monday and Friday
    ...
    let emailHtml = `<h1 style="color: #0f172a;">Informe Automático: Actividades Próxima Semana</h1>
    ...
    if (!hasEvents) {
      emailHtml += `<p style="color: #64748b; font-style: italic;">No hay actividades, reservas ni eventos programados para la próxima semana.</p>`;
    }
    ...
    await resend.emails.send({
      ...
      subject: `Resumen Semanal de Actividades (${startDate.toLocaleDateString('es-ES')} - ${endDate.toLocaleDateString('es-ES')})`,
      html: emailHtml,
    });
